...
def counter_panel(metrics: Counters) -> Panel:
    ...
    table.add_row('Tasks processed', f'[green]{metrics.tasks_processed_total}[/]')
    table.add_row('Tasks failed', f'[red]{metrics.tasks_failed_total}[/]')
    table.add_row('Processing rate', f'[cyan]{metrics.processing_rate:.2f}/sec[/]')
...
def status_spinner(metrics: Counters):
    text = f'[bold yellow]({metrics.tasks_failed_total} - {metrics.tasks_processed_total} / {metrics.tasks_total})[/]'
...
async def dashboard(metrics: Counters):
    ...
    with Live(layout, refresh_per_second=10, screen=True):
        while metrics.tasks_processed_total < metrics.tasks_total:
            ...
            layout['left'].update(Panel(status_spinner(metrics), title='Status'))
