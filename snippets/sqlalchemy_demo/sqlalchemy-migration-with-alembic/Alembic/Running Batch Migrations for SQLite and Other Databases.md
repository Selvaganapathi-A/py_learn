...
title: Running “Batch” Migrations for SQLite and Other Databases — Alembic 1.17.2
    documentation
---
...
The [`Operations.batch_alter_table()`](https://alembic.sqlalchemy.org/en/latest/ops.html#alembic.operations.Operations.batch_alter_table "alembic.operations.Operations.batch_alter_table") method provides the gateway to this process:
...
On other backends, we’d see the usual `ALTER` statements done as though there were no batch directive - the batch context by default only does the “move and copy” process if SQLite is in use, and if there are migration directives other than [`Operations.add_column()`](https://alembic.sqlalchemy.org/en/latest/ops.html#alembic.operations.Operations.add_column "alembic.operations.Operations.add_column") present, which is the one kind of column-level ALTER statement that SQLite supports.[`Operations.batch_alter_table()`](https://alembic.sqlalchemy.org/en/latest/ops.html#alembic.operations.Operations.batch_alter_table "alembic.operations.Operations.batch_alter_table") can be configured to run “move and copy” unconditionally in all cases, including on databases other than SQLite; more on this is below.

## Controlling Table Reflection

The [`Table`](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table "(in SQLAlchemy v2.0)") object that is reflected when “move and copy” proceeds is performed using the standard `autoload=True` approach. This call can be affected using the [`reflect_args`](https://alembic.sqlalchemy.org/en/latest/ops.html#alembic.operations.Operations.batch_alter_table.params.reflect_args "alembic.operations.Operations.batch_alter_table") and [`reflect_kwargs`](https://alembic.sqlalchemy.org/en/latest/ops.html#alembic.operations.Operations.batch_alter_table.params.reflect_kwargs "alembic.operations.Operations.batch_alter_table") arguments. For example, to override a [`Column`](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Column "(in SQLAlchemy v2.0)") within the reflection process such that a [`Boolean`](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.Boolean "(in SQLAlchemy v2.0)") object is reflected with the `create_constraint` flag set to `False`:
...
Another use case, add a listener to the [`Table`](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table "(in SQLAlchemy v2.0)") as it is reflected so that special logic can be applied to columns or types, using the [`column_reflect()`](https://docs.sqlalchemy.org/en/20/core/events.html#sqlalchemy.events.DDLEvents.column_reflect "(in SQLAlchemy v2.0)") event:
...
The reflection process may also be bypassed entirely by sending a pre-fabricated [`Table`](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table "(in SQLAlchemy v2.0)") object; see for an example.
...
The challenge this represents is that an unnamed constraint can’t by itself be targeted by the [`BatchOperations.drop_constraint()`](https://alembic.sqlalchemy.org/en/latest/ops.html#alembic.operations.BatchOperations.drop_constraint "alembic.operations.BatchOperations.drop_constraint") method. An unnamed FOREIGN KEY constraint is implicit whenever the [`ForeignKey`](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.ForeignKey "(in SQLAlchemy v2.0)") or [`ForeignKeyConstraint`](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.ForeignKeyConstraint "(in SQLAlchemy v2.0)") objects are used without passing them a name. Only on SQLite will these constraints remain entirely unnamed when they are created on the target database; an automatically generated name will be assigned in the case of all other database backends.

Within the scope of batch mode, this presents the issue that the [`BatchOperations.drop_constraint()`](https://alembic.sqlalchemy.org/en/latest/ops.html#alembic.operations.BatchOperations.drop_constraint "alembic.operations.BatchOperations.drop_constraint") method requires a constraint name in order to target the correct constraint.

In order to overcome this, the [`Operations.batch_alter_table()`](https://alembic.sqlalchemy.org/en/latest/ops.html#alembic.operations.Operations.batch_alter_table "alembic.operations.Operations.batch_alter_table") method supports a [`naming_convention`](https://alembic.sqlalchemy.org/en/latest/ops.html#alembic.operations.Operations.batch_alter_table.params.naming_convention "alembic.operations.Operations.batch_alter_table") argument, so that all reflected constraints, including foreign keys that are unnamed may be given a name, as described in [Integration of Naming Conventions into Operations, Autogenerate](https://alembic.sqlalchemy.org/en/latest/naming.html#autogen-naming-conventions). Usage is as follows:
...
The SQLAlchemy types [`Boolean`](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.Boolean "(in SQLAlchemy v2.0)") and [`Enum`](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.Enum "(in SQLAlchemy v2.0)") are part of a category of types known as “schema” types; this style of type creates other structures along with the type itself, most commonly (but not always) a CHECK constraint.
...
Changed in version 1.7: The [`BatchOperations.drop_column()`](https://alembic.sqlalchemy.org/en/latest/ops.html#alembic.operations.BatchOperations.drop_column "alembic.operations.BatchOperations.drop_column") operation can accept an `existing_type` directive where a “schema type” such as [`Boolean`](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.Boolean "(in SQLAlchemy v2.0)") and [`Enum`](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.Enum "(in SQLAlchemy v2.0)") may be specified such that an associated named constraint can be removed.
...
For **unnamed** CHECK constraints, these are still not automatically included as part of the batch process. Note that this limitation **includes** the CHECK constraints generated by the [`Boolean`](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.Boolean "(in SQLAlchemy v2.0)") or [`Enum`](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.Enum "(in SQLAlchemy v2.0)") datatypes, which up through SQLAlchemy 1.3 would generate CHECK constraints automatically and cannot be tracked to the reflected table, assuming they are generated in an unnamed way.
...
In the preceding sections, we’ve seen how much of an emphasis the “move and copy” process has on using reflection in order to know the structure of the table that is to be copied. This means that in the typical case, “online” mode, where a live database connection is present so that [`Operations.batch_alter_table()`](https://alembic.sqlalchemy.org/en/latest/ops.html#alembic.operations.Operations.batch_alter_table "alembic.operations.Operations.batch_alter_table") can reflect the table from the database, is required; the `--sql` flag **cannot** be used without extra steps.

To support offline mode, the system must work without table reflection present, which means the full table as it intends to be created must be passed to [`Operations.batch_alter_table()`](https://alembic.sqlalchemy.org/en/latest/ops.html#alembic.operations.Operations.batch_alter_table "alembic.operations.Operations.batch_alter_table") using [`copy_from`](https://alembic.sqlalchemy.org/en/latest/ops.html#alembic.operations.Operations.batch_alter_table.params.copy_from "alembic.operations.Operations.batch_alter_table"):
...
The syntax of batch mode is essentially that [`Operations.batch_alter_table()`](https://alembic.sqlalchemy.org/en/latest/ops.html#alembic.operations.Operations.batch_alter_table "alembic.operations.Operations.batch_alter_table") is used to enter a batch block, and the returned [`BatchOperations`](https://alembic.sqlalchemy.org/en/latest/ops.html#alembic.operations.BatchOperations "alembic.operations.BatchOperations") context works just like the regular [`Operations`](https://alembic.sqlalchemy.org/en/latest/ops.html#alembic.operations.Operations "alembic.operations.Operations") context, except that the “table name” and “schema name” arguments are omitted.

To support rendering of migration commands in batch mode for autogenerate, configure the [`EnvironmentContext.configure.render_as_batch`](https://alembic.sqlalchemy.org/en/latest/api/runtime.html#alembic.runtime.environment.EnvironmentContext.configure.params.render_as_batch "alembic.runtime.environment.EnvironmentContext.configure") flag in `env.py`:
...
This mode is safe to use in all cases, as the [`Operations.batch_alter_table()`](https://alembic.sqlalchemy.org/en/latest/ops.html#alembic.operations.Operations.batch_alter_table "alembic.operations.Operations.batch_alter_table") directive by default only takes place for SQLite; other backends will behave just as they normally do in the absence of the batch directives.

Note that autogenerate support does not include “offline” mode, where the [`Operations.batch_alter_table.copy_from`](https://alembic.sqlalchemy.org/en/latest/ops.html#alembic.operations.Operations.batch_alter_table.params.copy_from "alembic.operations.Operations.batch_alter_table") parameter is used. The table definition here would need to be entered into migration files manually if this is needed.
...
Note that also as is the case with SQLite, CHECK constraints need to be moved over between old and new table manually using the [`Operations.batch_alter_table.table_args`](https://alembic.sqlalchemy.org/en/latest/ops.html#alembic.operations.Operations.batch_alter_table.params.table_args "alembic.operations.Operations.batch_alter_table") parameter.